...
const Nav = () => {
  ...
  let menu = [];
  if (UserEmail === "Teacher@gmail.com") menu = Teacher;
  else if (UserEmail === "Student@gmail.com") menu = Student;
  else if (UserEmail === "Admin@gmail.com") menu = Admin;

  return (
    <nav className="bg-white dark:bg-gray-900 shadow-md w-full sticky top-0 z-[60] transition-colors duration-300 border-b dark:border-gray-800">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">

          {/* Logo */}
          <div className="text-2xl font-bold text-gray-800 dark:text-white">MyLogo</div>
                ...
                className={({ isActive }) =>
                  `rounded-2xl py-3 px-4 font-medium transition
                   ${
                     isActive
                       ? "bg-indigo-600 text-white"
                       : "text-gray-600 dark:text-gray-300 hover:bg-gradient-to-r from-indigo-500 to-purple-500 hover:text-white"
                   }`
                }
              >
                {item.item}
      ...
      {/* Mobile Sidebar Menu */}
      <div className={`md:hidden fixed top-0 right-0 w-80 h-full bg-white dark:bg-gray-900 shadow-xl z-50 transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
              ...
              className={({ isActive }) =>
                `block px-3 py-3 rounded-md font-medium transition
                 ${
                   isActive
                     ? "bg-indigo-600 text-white"
                     : "text-gray-700 dark:text-gray-300 hover:bg-gradient-to-r from-indigo-500 to-purple-500 hover:text-white"
                 }`
              }
            >
              {item.item}
