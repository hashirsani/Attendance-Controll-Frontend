...
const Attendance = () => {
  ...
  let array = [];
  if (selectedSubject === 'OS') array = OS;
  else if (selectedSubject === 'PF') array = PF;
  else if (selectedSubject === 'HCI') array = HCI;
  else if (selectedSubject === 'IDS') array = IDS;
  else if (selectedSubject === 'Psychology') array = Psychology;
  ...
  if (!selectedSubject) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white dark:bg-gray-950 transition-colors">
        <h2 className="text-xl text-gray-600 dark:text-gray-400">No Subject Selected</h2>
        <button onClick={() => navigate('/Subjects')} className="mt-4 text-blue-600 dark:text-blue-400 underline">Go Back</button>
      </div>
    );
  }

  return (
    <div className="bg-gray-50 dark:bg-gray-950 min-h-screen p-4 sm:p-6 md:p-10 transition-colors">
      
      {/* Header & Back Button */}
      <div className="max-w-5xl mx-auto mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100">{selectedSubject}</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm md:text-base">Attendance Dashboard & History</p>
        </div>
        <button 
          onClick={() => navigate('/Subjects')}
          className="w-full sm:w-auto px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-all"
        >
          ← Back to Subjects
        </button>
      </div>
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
        
        {/* Total Classes */}
        <div className="bg-white dark:bg-gray-900 p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
          <p className="text-gray-500 dark:text-gray-400 text-xs md:text-sm font-medium">Total Lectures</p>
          <p className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100 mt-2">{totalClasses}</p>
        </div>

        {/* Present */}
        <div className="bg-white dark:bg-gray-900 p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
          <p className="text-gray-500 dark:text-gray-400 text-xs md:text-sm font-medium">Lectures Taken</p>
          <p className="text-2xl md:text-3xl font-bold text-green-600 dark:text-green-400 mt-2">{presentCount}</p>
        </div>

        {/* Absent */}
        <div className="bg-white dark:bg-gray-900 p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
          <p className="text-gray-500 dark:text-gray-400 text-xs md:text-sm font-medium">Lectures Missed</p>
          <p className="text-2xl md:text-3xl font-bold text-red-500 dark:text-red-400 mt-2">{absentCount}</p>
        </div>

        {/* Percentage Card */}
        <div className="bg-white dark:bg-gray-900 p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 relative overflow-hidden">
          <p className="text-gray-500 dark:text-gray-400 text-xs md:text-sm font-medium">Attendance %</p>
          <div className="flex items-end gap-2 mt-2">
            <p className={`text-2xl md:text-3xl font-bold ${percentage < 75 ? 'text-red-600 dark:text-red-400' : 'text-indigo-600 dark:text-indigo-400'}`}>
              {percentage}%
            </p>
          </div>
          <div className={`mt-2 inline-block px-2 py-1 rounded text-[10px] md:text-xs font-bold ${getStatusColor()}`}>
            {percentage >= 75 ? "Safe" : "Low Attendance"}
          </div>
        </div>
      </div>
      ...
      {/* 2. PROGRESS BAR & POLICY */}
      <div className="max-w-5xl mx-auto bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 md:p-8 mb-8">
        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">Attendance Status</h3>
        
        {/* Progress Bar */}
        <div className="w-full bg-gray-200 dark:bg-gray-800 rounded-full h-3 md:h-4 mb-3">
          <div 
            className={`h-3 md:h-4 rounded-full transition-all duration-1000 ${percentage >= 75 ? 'bg-gradient-to-r from-green-400 to-green-600' : 'bg-gradient-to-r from-red-400 to-red-600'}`} 
            style={{ width: `${percentage}%` }}
          ></div>
        </div>
        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
          University policy requires <strong className="text-gray-700 dark:text-gray-200">75%</strong> attendance to sit in the final exam.
          {percentage < 75 && <span className="text-red-500 dark:text-red-400 font-bold ml-1">You are at risk!</span>}
        </p>
      </div>

      {/* 3. DETAILED LOG (HISTORY) - Scrollable Table for Mobile */}
      <div className="max-w-5xl mx-auto">
        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-6">Lecture History</h3>
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th className="p-4 text-sm font-semibold text-gray-600 dark:text-gray-300">Topic / Description</th>
                  <th className="p-4 text-sm font-semibold text-gray-600 dark:text-gray-300 text-center">Status</th>
                </tr>
              </thead>
              <tbody>
                {array.map((record) => (
                  <tr key={record.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="p-4 text-gray-800 dark:text-gray-200 font-medium text-sm md:text-base">{record.date}</td>
                    <td className="p-4 text-gray-500 dark:text-gray-400 text-sm md:text-base">{record.topic}</td>
                    <td className="p-4 text-center">
